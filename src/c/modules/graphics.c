...
void graphics_draw_steps_value(GContext *ctx, GRect bounds, GColor color, GBitmap *bitmap) {
  ...
  shoe_bitmap_box.size = gbitmap_get_bounds(data_get_green_step()).size;

  int text_width = graphics_text_layout_get_content_size(steps_buffer, 
      data_get_font(FontSizeSmall), steps_text_box, GTextOverflowModeTrailingEllipsis, GTextAlignmentRight).w;
  const int font_height = 14;
  const int padding = 5;
  steps_text_box.size = GSize(144, font_height);
  const int combined_width = shoe_bitmap_box.size.w + padding + text_width;

  steps_text_box.origin.x = bounds.size.w-36;
  steps_text_box.origin.y = 0;
  
  shoe_bitmap_box.origin.x = (bounds.size.w / 2) + (combined_width / 2) - shoe_bitmap_box.size.w;
  shoe_bitmap_box.origin.y = PBL_IF_RECT_ELSE(42, 70);

  graphics_context_set_text_color(ctx, color);
//   graphics_draw_text(ctx, steps_buffer, data_get_font(FontSizeSmall), 
//                      steps_text_box, GTextOverflowModeTrailingEllipsis, GTextAlignmentRight, NULL);

//   graphics_draw_bitmap_in_rect(ctx, bitmap, shoe_bitmap_box);
  
  GRect steps_rect = GRect(26, 39,
                     bounds.size.w-52, 15);
  graphics_draw_text(ctx, steps_buffer, data_get_font(FontSizeSmall), steps_rect, 
